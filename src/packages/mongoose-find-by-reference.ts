...
export function MongooseFindByReference(schema: Schema) {
  ...
  // Hook on the Schema
  schema.pre(
    ['find', 'findOne', 'findOneAndUpdate', 'findOneAndDelete', 'distinct', 'count'],
    async function (next) {
      /** 当前的 Model 们
       *  Current Models */
       ...
       */
      function transPath2RefPath(paths: string[], tSchema: Schema = schema): string[] {
      ...
      function flatten(dd: Dict, separator = '.', prefix = ''): Dict {
        ...
        for (const [k, v] of Object.entries(dd)) {
          ...
          if (
            ...
            const flatObject = flatten(v as Dict, separator, key)
      ...
      async function lookup(
        ...
        for (let [paths, value] of Object.entries(conditions)) {
          ...
          if (!schema.path(paths)) {
            const reduceResult = [...transPath2RefPath(paths.split('.')), value as any].reduceRight(
          ...
          if (Array.isArray(value))
            Object.assign(result, {
              [paths]: await Promise.all(
                value.map(async (v) => await lookup(currentPathsArray, v, cSchema))
              )
            })
          else if (
            typeof value === 'object' &&
      ...
      obj._conditions = await lookup([], (this as any)._conditions)
      next()
    }
  )
}
